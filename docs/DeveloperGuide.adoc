...
image::DeleteIngredientSequenceDiagram.png[width="630"]
...
* Decision: Alternative 1 was selected since aliases are implemented as a shortcut to the long commands (e.g. `db` for `deleteBooking`, `dm` for `deleteMember`) and no longer inconvenient for users.
Since 4 panels are implemented in U/I, this provides easy cross-reference across linked records. It is hard for the user to detect an active list at first glance, since all lists are already displayed. With chosen implementation, specific delete command can be called without a prior command to mark the list as active (e.g.  `deleteIngredient` can be called directly to make changes to Ingredient list), providing convenience to user.
...
image::activity_diagram_deleteingredient.png[width="400"]
{sp} +
