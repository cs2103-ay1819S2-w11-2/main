...
****
• `INGREDIENT_NAME` and `INGREDIENT_UNIT` must contain only alphabets and spaces, and must contain at least 2 characters of the alphabet. (i.e, `in/c` is not accepted while `in/ch` is). All leading whitespaces for `INGREDIENT_NAME` and `INGREDIENT_UNIT` will be trimmed.
...
• There should be at least 1 ingredient `iq/INDEX&INGREDIENT_QUANTITY_IN_RECIPE` input but there cannot be duplicate ingredients in the recipe, i.e. there cannot be duplicate ingredient indexes inputted. (`addRecipe r/cheese fries iq/1&1 iq/2&1 iq/1&10` is rejected).
...
.  `addIngredient in/cheese u/slices q/20` +
   ...
   * These commands add cheese and fries into the ingredient panel, and cheese fries into recipe panel. For each serving of cheese fries, 1 slice of cheese and 1
    ...
    Overall, there can only be 10 servings of cheese fries prepared. Upon success, panels should look like this:
+
.Successfully adding 1 recipe containing 2 ingredients, displaying the number of servings possible.
image::addRecipe-successful.png[width=700]
...
image::addRecipe-restockIngredient.png[width=700]
